...
export class MaterialsService {
  ...
  fetchMaterials(buildingID, mode, materialID) {
    ...
    if (materialID === null) {
      ...
      console.log('https://www.calgrois.co.za/api/v1/materials/' + materialID + '/' + this.query);
      this.http.get<MaterialDetail[]>('https://www.calgrois.co.za/api/v1/materials/' + materialID + '/' + this.query).subscribe(
        resp => {
          this.query = ''
          if (resp) {
            this.materialListChanged.emit(resp);
